...
} from "../api/user/delete-user-by-id";
import {
  UpdateUserByIdRequest,
  UpdateUserByIdResponse,
} from "../api/user/update-user-by-id";
...
export const createUser = async ({
  ...
  const response = await api.post<PageResponse<User>>("/user", {
    username,
    email,
    name,
    last_name,
...
export const getAllUsers = async (
  { perPage, page }: GetAllUsersRequest = { perPage: 10, page: 1 }
